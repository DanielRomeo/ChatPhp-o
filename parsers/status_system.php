...
if (isset($_POST['action']) && $_POST['action'] == "status_post"){
	...
	//$row = mysqli_fetch_row($query);			// i think  i have an error over here:
	...
	for($i = 0; $i < count($friends); $i++){
		...
		/**/
		$note = ' <li class="list-group-item">'.$log_username. ', Posted something </li> '; 
...
?>                        
<?php 	// POSTING THE STATUS REPLY TO THE DATABASE:
...
if (isset($_POST['action']) && $_POST['action'] == "delete_status"){
	if(!isset($_POST['statusid']) || $_POST['statusid'] == ""){
		mysqli_close($db_conx);
		echo "status id number is missing";
		exit();
	}
