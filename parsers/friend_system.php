<?php
include_once("../xphp_includes/check_login_status.php");
...
?>
<?php  // this block of php processes the Accept and Rejject buttons:

// check if either An action(accept or reject is set) and if it holds the id and username:
// if not then, throw an errror 404:
// else continue:
if (isset($_POST['action']) && isset($_POST['reqid']) && isset($_POST['user1'])){
	...
	if($exist_count[0] < 1){
		...
		// run to check if these people are friends alredy :
		// if yess: throw an error;
		// if not: (accept) their friendship invitation:
	if($_POST['action'] == "accept"){
		...
		// simply delete friendship INVITATION from INVITATION(friends) table:
